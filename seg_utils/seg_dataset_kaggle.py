...
def load_gt_kaggle(annoDir):
    ...
    for anno_file in files:
        mask = cv2.imread(os.path.join(annoDir, anno_file), 0)
...
class NucleiCell(Dataset):
    ...
    def load_img_ids(self):
        path_list = os.listdir(self.imgDirectory)
    ...
    def load_annotation(self, img_file):
        ...
        return bboxes, labels, masks, barcode

    def __getitem__(self, item):
        img = self.load_img(item)
        # barcode = self.img_files[item].split('/')[-1].split('.')[0]
